...
def test_coactivation(temp_state_file, reset_state):
    ...
    register_file("primary/file.md", keywords=["trigger"], coactivate_with=["related/file.md"])
...
def test_tool_spec_exists():
    ...
    assert len(tool.functions) == 11  # All functions registered (added flush_pending_updates)
